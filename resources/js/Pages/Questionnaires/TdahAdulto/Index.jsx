...
export default function TdahAdultoIndex({
    ...
    const handleSearch = () => {
        router.get(route('questionnaires.tdah-adulto.index'), buildParams(), { preserveState: true, replace: true });
    };

    const clearFilters = () => {
        setSearch(''); setDateFrom(''); setDateTo(''); setClinica(''); setSelectedTeam(''); setSortField(''); setSortDirection('desc');
        router.get(route('questionnaires.tdah-adulto.index'));
    };
    ...
    const handleSort = (field) => {
        ...
        router.get(route('questionnaires.tdah-adulto.index'), params, { preserveState: true, replace: true });
    };
            ...
            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors duration-200" onClick={() => handleSort(field)}>
                <div className="flex items-center space-x-1">
                    ...
                    <div className="flex flex-col">
                        <svg className={`w-3 h-3 ${isActive && sortDirection === 'asc' ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400'}`} fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M14.707 12.707a1 1 0 01-1.414 0L10 9.414l-3.293 3.293a1 1 0 01-1.414-1.414l4-4a1 1 0 011.414 0l4 4a1 1 0 010 1.414z" clipRule="evenodd" /></svg>
                        <svg className={`w-3 h-3 -mt-1 ${isActive && sortDirection === 'desc' ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400'}`} fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
                    </div>
                </div>
            </th>
        );
            ...
            header={
                <div className="flex justify-between items-center">
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            <h2 className="font-bold text-xl text-gray-800 dark:text-gray-200 leading-tight">TDAH Adulto</h2>
                            <p className="text-xs text-gray-600 dark:text-gray-400">Escala ASRS-18</p>
                        </div>
                    </div>
                                            ...
                                            <div className="p-4">
                                                <div className="flex justify-between items-start mb-3">
                                                    ...
                                                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 dark:bg-indigo-900 text-indigo-800 dark:text-indigo-200">
                                                        A: {questionnaire.parte_a_total ?? '-'} / B: {questionnaire.parte_b_total ?? '-'}
                                                    </span>
                                                </div>
                                ...
                                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    <thead className="bg-gray-50 dark:bg-gray-700">
                                        <tr>
                                            ...
                                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Parte A / B</th>
                                    ...
                                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                        {questionnaires.data && questionnaires.data.length > 0 ? (
                                            questionnaires.data.map((questionnaire, index) => (
                                                <tr key={questionnaire.id} className={`hover:bg-gradient-to-r hover:from-indigo-50 hover:to-blue-50 dark:hover:from-indigo-900/10 dark:hover:to-blue-900/10 transition-all duration-200 ${index % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50/50 dark:bg-gray-850'}`}>
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-gradient-to-r from-indigo-100 to-blue-200 dark:from-indigo-900 dark:to-blue-800 text-indigo-800 dark:text-indigo-200 border border-indigo-200 dark:border-indigo-700 shadow-sm">
                                                            {questionnaire.parte_a_total ?? '-'} / {questionnaire.parte_b_total ?? '-'}
                                                        </span>
                                                    </td>
                                                    <td className="px-6 py-4">
                                ...
                                <div className="mt-6 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-750 rounded-xl p-4 border border-gray-200 dark:border-gray-600">
                                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                                        <div className="flex items-center text-sm font-medium text-gray-700 dark:text-gray-300 text-center sm:text-left">
                                            Mostrando <span className="font-bold text-indigo-600 dark:text-indigo-400 mx-1">{questionnaires.from || 0}</span> a <span className="font-bold text-indigo-600 dark:text-indigo-400 mx-1">{questionnaires.to || 0}</span> de <span className="font-bold text-indigo-600 dark:text-indigo-400 mx-1">{questionnaires.total || 0}</span> resultados
                                        </div>
                                        <div className="flex flex-wrap justify-center gap-1">
                                            {questionnaires.links.map((link, index) => {
                                                ...
                                                return (<button key={index} onClick={handlePaginationClick} className={`inline-flex items-center px-4 py-2 text-sm font-medium border rounded-lg transition-all duration-200 shadow-sm hover:shadow ${link.active ? 'bg-gradient-to-r from-indigo-500 to-blue-600 dark:from-indigo-600 dark:to-blue-700 text-white border-indigo-500 dark:border-indigo-600 shadow-md' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-indigo-300 dark:hover:border-indigo-600'}`} dangerouslySetInnerHTML={{ __html: link.label }} />);
                                            })}
                                        </div>
                                    </div>
                                </div>
