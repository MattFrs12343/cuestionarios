...
export default function RastreioCognitivoIndex({
    ...
    const handleSearch = () => {
        router.get(route('questionnaires.rastreio-cognitivo.index'), buildParams(), {
            preserveState: true,
            replace: true
        });
    };
    ...
    const clearFilters = () => {
        ...
        router.get(route('questionnaires.rastreio-cognitivo.index'));
    };
    ...
    const handleSort = (field) => {
        ...
        router.get(route('questionnaires.rastreio-cognitivo.index'), params, {
            preserveState: true,
            replace: true
        });
    };
    ...
    return (
                    ...
                    <div className="flex items-center space-x-3">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-gray-600 dark:text-gray-400">
                                Protocolo de Rastreio Cognitivo em Consulta
                            </p>
                        </div>
                    </div>
                    <Link
        ...
        >
            <Head title="Rastreio Cognitivo (MoCA)" />
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
                        <div className="bg-gradient-to-br from-teal-500 to-teal-600 dark:from-teal-600 dark:to-teal-700 rounded-xl shadow-lg p-5 text-white transform hover:scale-105 transition-transform duration-200">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-teal-100 text-xs font-medium uppercase tracking-wide">Total de Exames</p>
                        ...
                        <div className="bg-gradient-to-br from-cyan-500 to-cyan-600 dark:from-cyan-600 dark:to-cyan-700 rounded-xl shadow-lg p-5 text-white transform hover:scale-105 transition-transform duration-200">
                            <div className="flex items-center justify-between">
                                <div>
                                    <p className="text-cyan-100 text-xs font-medium uppercase tracking-wide">Este Mês</p>
                            ...
                            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 mb-6">
                                <div>
                                    <h3 className="text-2xl font-bold text-gray-900 dark:text-gray-100 flex items-center">
                                        <span className="w-1 h-8 bg-gradient-to-b from-teal-500 to-cyan-600 rounded-full mr-3"></span>
                                        Questionários de Rastreio Cognitivo
                                    </h3>
                                    {currentTeam ? (
                                ...
                                {can.create && (
                                    <Link
                                        href={route('questionnaires.rastreio-cognitivo.create')}
                                        className="inline-flex items-center justify-center bg-gradient-to-r from-teal-500 to-cyan-600 dark:from-teal-600 dark:to-cyan-700 hover:from-teal-600 hover:to-cyan-700 dark:hover:from-teal-700 dark:hover:to-cyan-800 text-white font-semibold py-3 px-6 rounded-lg shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200"
                                    >
                            ...
                            <div className="mb-6 p-5 bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-800 border border-gray-200 dark:border-gray-600 rounded-xl shadow-sm transition-colors duration-200">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
                                    ...
                                    <div className="flex gap-2">
                                        <button onClick={handleSearch} className="flex-1 px-4 py-2 bg-teal-500 dark:bg-teal-600 text-white text-sm font-medium rounded-md hover:bg-teal-600 dark:hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-teal-500 dark:focus:ring-teal-600 transition-colors duration-200">Buscar</button>
                                            ...
                                            <div className="p-4">
                                                <div className="flex justify-between items-start mb-3">
                                                    ...
                                                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-teal-100 dark:bg-teal-900 text-teal-800 dark:text-teal-200">
                                                        Total: {questionnaire.pontuacao_total ?? '-'}/30
                                                    </span>
                                                </div>
                                    ...
                                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                        {questionnaires.data && questionnaires.data.length > 0 ? (
                                            questionnaires.data.map((questionnaire, index) => (
                                                <tr key={questionnaire.id} className={`hover:bg-gradient-to-r hover:from-teal-50 hover:to-cyan-50 dark:hover:from-teal-900/10 dark:hover:to-cyan-900/10 transition-all duration-200 ${index % 2 === 0 ? 'bg-white dark:bg-gray-800' : 'bg-gray-50/50 dark:bg-gray-850'}`}>
                                                    <td className="px-6 py-4">
                                                        <div className="flex items-center">
                                                            <div className="w-10 h-10 bg-gradient-to-br from-teal-100 to-cyan-100 dark:from-teal-900/30 dark:to-cyan-900/30 rounded-lg flex items-center justify-center mr-3 flex-shrink-0">
                                ...
                                <div className="mt-6 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-750 rounded-xl p-4 border border-gray-200 dark:border-gray-600">
                                    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
                                        <div className="flex items-center text-sm font-medium text-gray-700 dark:text-gray-300 text-center sm:text-left">
                                            Mostrando <span className="font-bold text-teal-600 dark:text-teal-400 mx-1">{questionnaires.from || 0}</span> a <span className="font-bold text-teal-600 dark:text-teal-400 mx-1">{questionnaires.to || 0}</span> de <span className="font-bold text-teal-600 dark:text-teal-400 mx-1">{questionnaires.total || 0}</span> resultados
                                        </div>
                                        <div className="flex flex-wrap justify-center gap-1">
                                            {questionnaires.links.map((link, index) => {
                                                ...
                                                const handlePaginationClick = (e) => {
                                                    ...
                                                    router.get(route('questionnaires.rastreio-cognitivo.index'), params, { preserveState: true, replace: true });
                                                };
                                                return (
                                                    <button
                                                        key={index}
                                                        onClick={handlePaginationClick}
                                                        className={`inline-flex items-center px-4 py-2 text-sm font-medium border rounded-lg transition-all duration-200 shadow-sm hover:shadow ${link.active ? 'bg-gradient-to-r from-teal-500 to-cyan-600 dark:from-teal-600 dark:to-cyan-700 text-white border-teal-500 dark:border-teal-600 shadow-md' : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-teal-300 dark:hover:border-teal-600'}`}
